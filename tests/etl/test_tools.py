...
def _populate_test_data_helper(repo: Dolt, mens: pd.DataFrame, womens: pd.DataFrame, branch: str = 'main'):
...
def test_load_to_dolt_new_branch(initial_test_data):
    ...
    assert current_branch.name == 'main' and [b.name for b in current_branches] == ['main', test_branch]
