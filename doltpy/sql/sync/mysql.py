...
def get_target_writer(
    ...
    :return:
    """
    return get_target_writer_helper(
        engine, upsert_helper, update_on_duplicate, clean_types
    )
...
def upsert_helper(table: Table, data: List[dict]):
    ...
    update_dict = {
        el.name: el for el in insert_statement.inserted if not el.primary_key
    }
